...
//  current_scope_->InsertType("void",
//  std::make_shared<Type>(Type::Kind::VOID)); current_scope_->InsertType("i32",
//  std::make_shared<Type>(Type::Kind::I32)); current_scope_->InsertType("i64",
//  std::make_shared<Type>(Type::Kind::I64)); current_scope_->InsertType("f32",
//  std::make_shared<Type>(Type::Kind::F32)); current_scope_->InsertType("f64",
//  std::make_shared<Type>(Type::Kind::F64)); current_scope_->InsertType("bool",
//  std::make_shared<Type>(Type::Kind::BOOL));
//  current_scope_->InsertType("char",
//  std::make_shared<Type>(Type::Kind::CHAR));
...
///{ /  auto hir_file = std::make_unique<hir::File>(); /  for (auto& ext :
///file->externs) { /    auto decl = InsertFnDecl(true, ext->proto); /
///hir_file->externs.emplace_back( /        new hir::Extern(ext->Begin(),
///ext->End(), decl)); /  } /  for (auto& fn : file->fn_decls) { /    auto decl
///= InsertFnDecl(false, fn->proto); /    hir_file->fn_decls.push_back( /
...
///- /   * 1)); */ /  /*   } else { */ /  /*     throw
///CompileError::Create("func %s is not terminated", */ /  /*
///hirDecl->decl->name.c_str()); */ /  /*   } */ /  /* } */
////
////  return std::move(block);
////}
////
// void Checker::CheckStmt(const std::unique_ptr<ast::Stmt>& stmt) {
...
//}
//
// void Checker::CheckExpr(const std::unique_ptr<ast::Expr>& expr) {
...
//}
//
// void Checker::CheckRet(const std::unique_ptr<ast::RetStmt>& stmt) {
...
//}
//
// void Checker::CheckVarDecl(const std::unique_ptr<ast::VarDeclStmt>& stmt) {
...
//}
//
// void Checker::CheckAssign(const std::unique_ptr<ast::AssignStmt>& stmt) {
//  std::string name = stmt->name->val;
...
//    throw LocError::Create(stmt->Begin(), "%s is declared as mutable
//    variable",
//                           name.c_str());
//  }
//  CheckExpr(stmt->expr);
//}
//
// void Checker::CheckIf(const std::unique_ptr<ast::If>& stmt) {
//  CheckExpr(stmt->cond);
...
//}
//
// void Checker::CheckBlock(const std::unique_ptr<ast::Block>& block) {
//  OpenScope();
...
////  auto decl = std::make_shared<Decl>(
////      name, exp->Ty(), decl_stmt->is_let ? Decl::Kind::LET :
///Decl::Kind::VAR); /  current_scope_->InsertDecl(name, decl); /  /*
///decl->Debug(); */ /  /* DebugScope(); */ /  return
///std::make_unique<hir::VarDeclStmt>(begin, decl, std::move(exp));
////}
////
...
////  }
////  if (decl->IsFunc()) {
////    throw LocError::Create(assign_stmt->Begin(), "%s is declared as
///function", /                           name.c_str()); /  } /  if
///(!decl->IsAssignable()) { /    throw LocError::Create(assign_stmt->Begin(),
...
///{ /  auto begin = if_stmt->Begin(); /  auto end = if_stmt->End(); /  auto
///cond = MakeExpr(std::move(if_stmt->cond)); /  if (!cond->Ty()->IsBool()) { /
///throw LocError::Create(cond->Begin(), "non bool if cond"); /  } /  auto block
///= CheckBlock(std::move(if_stmt->block));
////
////  if (if_stmt->HasElse()) {
////    if (if_stmt->IsElseIf()) {
...
////      auto els =
///CheckBlock(unique_cast<ast::Block>(std::move(if_stmt->els))); /      return
///std::make_unique<hir::If>(begin, end, std::move(cond), / std::move(block),
///std::move(els)); /    } /  } /  return std::make_unique<hir::If>(begin, end,
...
////}
//
//// std::unique_ptr<hir::Expr> Checker::MakeExpr(std::unique_ptr<ast::Expr>
///expr) / { /  switch (expr->ExprKind()) { /    case ast::Expr::Kind::LIT: { /
///return MakeLit(unique_cast<ast::Lit>(std::move(expr))); /    } /    case
///ast::Expr::Kind::CALL: { /      auto call_expr =
///unique_cast<ast::CallExpr>(std::move(expr)); /      auto begin =
///call_expr->Begin(); /      auto end = call_expr->End();
////
////      auto decl = LookupDecl(call_expr->ident->val);
////      if (decl == nullptr) {
////        throw LocError::Create(begin, "undefined function %s",
...
//// std::unique_ptr<hir::Expr> Checker::TryExprTy(std::unique_ptr<hir::Expr>
//// expr,
...
////          if (*var->decl->type != *ty) {
////            throw LocError::Create(expr->Begin(), "unmatched variable
///type"); /          } /          return std::move(expr); /        } break; /
///case hir::Value::Kind::CONSTANT: { /          // Constants can be casted
///implicitly /          auto cons =
///unique_cast<hir::Constant>(std::move(expr)); /          return
///TryConstantTy(std::move(cons), ty); /        } break; /      } /    } break;
////
...
////}
////
//// std::unique_ptr<hir::Constant> Checker::MakeLit(std::unique_ptr<ast::Lit>
//// lit) {
...
////    if (n <= INT64_MAX) {
////      return std::make_unique<hir::IntConstant>(lit->Begin(), lit->End(),
///n); /    } else { /      throw LocError::Create(lit->Begin(), "overflow
///int64"); /    } /  } catch (std::out_of_range e) { /    throw
///LocError::Create(lit->Begin(), "out of range"); /  } catch
///(std::invalid_argument e) { /    throw LocError::Create(lit->Begin(),
...
////}
//
// void Checker::Check(const std::unique_ptr<ast::File>& file) {
...
//}
//
// void Checker::CheckFnDecl(const std::unique_ptr<ast::FnDecl>& fn) {
//  current_func_ = ast_decl_[fn.get()];
//  OpenScope();
//  for (auto& arg : fn->proto->args->list) {
//    // arg-name duplication is already checked in parser
//    auto decl = std::make_shared<Decl>(arg->name->val,
//    LookupType(arg->ty->val),
//                                       Decl::Kind::ARG);
...
//  }
//  auto fn_type = std::make_shared<FuncType>(std::move(args),
//  std::move(ret_ty)); Decl::Kind kind = is_ext ? Decl::Kind::EXT :
//  Decl::Kind::FN; auto decl = std::make_shared<Decl>(proto->name->val,
//  fn_type, kind); current_scope_->InsertDecl(proto->name->val, decl); return
//  decl;
//}
//
